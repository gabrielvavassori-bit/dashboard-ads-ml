...
def _sales_intelligence_collect_item_meta(latest_payload: dict) -> dict[str, dict]:
    ...
    for raw_code, raw_sale in sales_items.items():
        ...
        meta[code] = {
            ...
            "familyId": str(sale.get("family_id") or sale.get("familyId") or "").strip(),
    ...
    for raw in ads_rows:
        ...
        if not meta[code]["familyId"]:
            meta[code]["familyId"] = str(raw.get("family_id") or raw.get("familyId") or "").strip()
        if not meta[code]["familyName"]:
            meta[code]["familyName"] = str(raw.get("family_name") or raw.get("familyName") or "").strip()
        if not meta[code]["userProductId"]:
...
def _build_sales_intelligence_memory_data(user, link) -> tuple[dict | None, str]:
    ...
    if not daily_rows and item_meta:
        for code, meta in item_meta.items():
            ...
            daily_rows.append({
                ...
                "family_id": str(meta.get("familyId") or "").strip(),
    ...
    for daily in daily_rows:
        ...
        sales_rows.append({
            ...
            "total": gross,
            "buyerPriceIncrease": _number(daily.get("buyer_price_increase") or daily.get("buyerPriceIncrease")),
            "sellingFee": _number(daily.get("selling_fee") or daily.get("sellingFee")),
            "installmentFee": _number(daily.get("installment_fee") or daily.get("installmentFee")),
            "shippingRevenue": _number(daily.get("shipping_revenue") or daily.get("shippingRevenue")),
            "shippingFee": _number(daily.get("shipping_fee") or daily.get("shippingFee")),
            "shippingExchangeCost": _number(daily.get("shipping_exchange_cost") or daily.get("shippingExchangeCost")),
            "shippingMeasurementCost": _number(daily.get("shipping_measurement_cost") or daily.get("shippingMeasurementCost")),
            "shippingWeightDifferenceCost": _number(daily.get("shipping_weight_difference_cost") or daily.get("shippingWeightDifferenceCost")),
            "discounts": _number(daily.get("discounts")),
            "refunds": _number(daily.get("refunds")),
            "billingMonth": month,
            "adsSale": "",
            "sku": sku,
            "mlb": code,
            "title": title,
            "familyId": str(daily.get("family_id") or meta.get("familyId") or "").strip(),
